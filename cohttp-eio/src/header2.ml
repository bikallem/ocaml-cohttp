...
let header decode encode name = { name; decode; encode }
...
(* Transfer-Encoding header.
   TODO bikal ensure that elements maintain the order on encoding/decoding. `chunked must applied
   the last
*)
...
let replace t { name; encode; _ } v =
  ...
  let[@tail_mod_cons] rec aux = function
    | [] -> []
    | [ (name', _) ] as l ->
        if String.equal name name' then [ (name', v) ] else l
    | [ (name1, v1); (name2, v2) ] -> (
        match (String.equal name name1, String.equal name name2) with
        | true, true -> [ (name, v) ]
        | true, false -> [ (name, v); (name2, v2) ]
        | false, true -> [ (name1, v1); (name, v) ]
        | false, false -> [ (name1, v1); (name2, v2) ])
    | (name1, v1) :: (name2, v2) :: tl -> (
        match (String.equal name name1, String.equal name name2) with
        | true, true -> (name, v) :: aux tl
        | true, false -> (name, v) :: (name2, v2) :: aux tl
        | false, true -> (name1, v1) :: (name, v) :: aux tl
        | false, false -> (name1, v1) :: (name2, v2) :: aux tl)
  in
  aux t
